//! Official MSVC Build Tools — install, update, uninstall, and validate operations.
...
use crate::types::{
    MsvcLifecycleStage, MsvcOperationKind, MsvcOperationOutcome, MsvcRuntimeKind,
    MsvcValidationStatus, MsvcRequest, OfficialInstallerMode,
};
...
fn write_runtime_state(
    ...
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| CoreError::fs("create_dir_all", &path, e))?;
    }
    ...
    .map_err(|e| CoreError::Other(format!("failed to serialize runtime state: {e}")))?;
    fs::write(&path, content)
        .map_err(|e| CoreError::fs("write", &path, e))?;
    Ok(())
}

fn write_installed_state(
    tool_root: &Path,
    state: &OfficialInstalledState,
) -> spoon_core::Result<()> {
    ...
    let content = serde_json::to_string_pretty(state)
        .map_err(|e| CoreError::Other(format!("failed to serialize installed state: {e}")))?;
    fs::write(&path, content)
        .map_err(|e| CoreError::fs("write", &path, e))?;
    Ok(())
}
...
async fn cache_bootstrapper(
    ...
    if let Some(local) = source.strip_prefix("file:///") {
        fs::copy(local, &destination)
            .map_err(|e| CoreError::external(format!("failed to copy bootstrapper from {local}"), e))?;
    } else if Path::new(&source).exists() {
        fs::copy(&source, &destination)
            .map_err(|e| CoreError::external(format!("failed to copy bootstrapper from {source}"), e))?;
    } else {
        let client = crate::common::http_client(proxy)?;
        spoon_core::check_token_cancel(cancel)?;
        let mut response = client
            .get(&source)
            .send()
            .await
            .map_err(|err| CoreError::network(&source, err))?
            .error_for_status()
            .map_err(|err| CoreError::network(&source, err))?;
        ...
        let mut file = fs::File::create(&destination)
            .map_err(|e| CoreError::fs("create", &destination, e))?;
...
fn write_command_metadata(
    ...
    .map_err(|e| CoreError::Other(format!("failed to serialize command metadata: {e}")))?;
    fs::write(&path, content)
        .map_err(|e| CoreError::fs("write", &path, e))?;
    Ok(())
}
...
fn write_official_canonical_state(
    tool_root: &Path,
    ...
    let previous = read_canonical_state(&layout);
    ...
    write_canonical_state(&layout, &state)
        .map_err(|e| CoreError::Other(format!("failed to write canonical state: {e}")))
}
...
async fn run_official_action_async(
    ...
    fs::create_dir_all(paths::official_msvc_cache_root(tool_root))
        .map_err(|e| CoreError::fs("create_dir_all", paths::official_msvc_cache_root(tool_root), e))?;
        ...
        && !installed_state.exists()
    {
        ...
        return Ok(MsvcOperationOutcome {
            kind: "msvc_operation",
            runtime: MsvcRuntimeKind::Official,
    ...
    if matches!(action, OfficialAction::Uninstall) {
        ...
        if runtime_state.exists() {
            fs::remove_file(&runtime_state)
                .map_err(|e| CoreError::fs("remove_file", &runtime_state, e))?;
        }
        if installed_state.exists() {
            fs::remove_file(&installed_state)
                .map_err(|e| CoreError::fs("remove_file", &installed_state, e))?;
        }
    ...
    write_official_canonical_state(
        tool_root,
        match action {
            OfficialAction::Install => MsvcOperationKind::Install,
            OfficialAction::Update => MsvcOperationKind::Update,
            OfficialAction::Uninstall => MsvcOperationKind::Uninstall,
        },
        true,
        Some(mode),
        None,
...
fn official_validate_toolchain_layout(tool_root: &Path) -> spoon_core::Result<(PathBuf, Vec<String>)> {
...
fn official_write_validation_workspace_scripts(
    cpp_root: &Path,
    source: &Path,
    output: &Path,
    compiler: &Path,
    ...
    fs::write(&build_script, script)
        .map_err(|err| CoreError::fs("write", &build_script, err))?;
...
pub async fn validate_official_toolchain_async(
    request: &MsvcRequest,
) -> spoon_core::Result<MsvcOperationOutcome> {
    let tool_root = request.root.as_path();
    let (instance_root, mut lines) = official_validate_toolchain_layout(tool_root)?;
    ...
    fs::create_dir_all(&validate_root)
        .map_err(|e| CoreError::Other(format!("failed to create {}: {e}", validate_root.display())))?;
    lines.push(format!(
    ...
    fs::create_dir_all(&cpp_root)
        .map_err(|e| CoreError::Other(format!("failed to create {}: {e}", cpp_root.display())))?;
    fs::create_dir_all(&rust_root)
        .map_err(|e| CoreError::Other(format!("failed to create {}: {e}", rust_root.display())))?;
    ...
    official_write_validation_workspace_scripts(
        &cpp_root,
        &source,
        &output,
        &compiler,
    ...
    let run_output = Command::new(&output)
        ...
        .output()
        .map_err(|err| {
            CoreError::Other(format!("failed to run {}: {err}", output.display()))
        })?;
    ...
    let layout = RuntimeLayout::from_root(tool_root);
    let previous = read_canonical_state(&layout);
    let detected = detect_installed_state(&instance_root);
    let state = MsvcCanonicalState {
        runtime_kind: MsvcRuntimeKind::Official,
        installed: true,
        version: detected.version,
