...
fn read_file_table(
    ...
    let rows = package
        .select_rows(query)
        .map_err(|err| BackendError::external("querying MSI File table", err))?;
    for row in rows {
...
fn read_component_table(
    ...
    let rows = package
        .select_rows(query)
        .map_err(|err| BackendError::external("querying MSI Component table", err))?;
    for row in rows {
...
fn read_directory_table(
    ...
    let rows = package
        .select_rows(query)
        .map_err(|err| BackendError::external("querying MSI Directory table", err))?;
    for row in rows {
...
fn read_media_table(package: &mut msi::Package<std::fs::File>) -> Result<Vec<MediaEntry>> {
    ...
    let rows = package
        .select_rows(query)
        .map_err(|err| BackendError::external("querying MSI Media table", err))?;
    for row in rows {
...
pub fn read_msi_cab_names(msi_path: &Path) -> Result<Vec<String>> {
    let mut package = msi::open(msi_path).map_err(|err| {
        BackendError::external(format!("opening MSI file '{}'", msi_path.display()), err)
    })?;
...
fn extract_cab<R: Read + io::Seek>(
    ...
    let mut cabinet =
        cab::Cabinet::new(reader).map_err(|err| BackendError::external("parsing CAB file", err))?;
    ...
    for cab_file_name in &file_names {
        ...
        std::fs::create_dir_all(&full_dir)
            .map_err(|err| BackendError::fs("create", &full_dir, err))?;
        let full_path = full_dir.join(&actual_name);
        if full_path.exists() {
            continue;
        }

        let mut reader = cabinet.read_file(cab_file_name).map_err(|err| {
            BackendError::external(format!("reading '{}' from CAB", cab_file_name), err)
        })?;
        let mut out = std::fs::File::create(&full_path)
            .map_err(|err| BackendError::fs("create", &full_path, err))?;
        io::copy(&mut reader, &mut out)
            .map_err(|err| BackendError::fs("write", &full_path, err))?;
        extracted += 1;
    }
...
pub fn extract_msi_with_staged_cabs(
    ...
    for media in &media_entries {
        ...
        let cab_file = std::fs::File::open(&cab_path)
            .map_err(|err| BackendError::fs("open", &cab_path, err))?;
        ...
        .map_err(|err| err.context(format!("extracting staged CAB '{}'", cab_path.display())))?;
        found_external = true;
    }
