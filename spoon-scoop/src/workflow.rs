...
pub fn plan_package_action_with_display(
    ...
    let resolved_manifest = tool_root
        ...
        .and_then(|root| crate::manifest::resolve_manifest_sync(root, package_name));
...
pub async fn acquire_assets(
    ...
    for asset in assets {
        ...
        if let Some(sender) = events {
            sender.send(spoon_core::SpoonEvent::Stage(spoon_core::StageEvent::started(
                spoon_core::LifecycleStage::Acquiring,
            )));
            sender.send(spoon_core::SpoonEvent::Progress(spoon_core::ProgressEvent::bytes(
...
pub async fn install_package<P: ScoopPorts>(
    ...
    // Resolve manifest
    let resolved = crate::resolve_package_manifest(layout, package_name)
        .await
        .ok_or_else(|| ScoopError::ManifestUnavailable)?;

    // Load manifest and resolve source
    let manifest_value = crate::load_manifest_value(&resolved.manifest_path)
        .await
        .map_err(|e| ScoopError::Other(format!("failed to load manifest: {e}")))?;
    let source = crate::resolve_package_source(&manifest_value)?;
    ...
    if let Some(sender) = events {
        sender.send(spoon_core::SpoonEvent::Finished(spoon_core::FinishEvent::success(Some(
...
pub async fn execute_package_action_streaming<P: ScoopPorts>(
    ...
    cancel: Option<&spoon_core::CancellationToken>,
    ports: &P,
    mut emit: impl FnMut(spoon_core::SpoonEvent) + Send + 'static,
...
/// Reapply command surface (shims) for an installed package.
///
/// Reads the installed state, resolves the latest manifest, removes old shims,
/// writes new shims to the global shims directory, and updates the install state.
pub async fn reapply_command_surface<P: ScoopPorts>(
    layout: &ScoopLayout,
    shims_root: &Path,
    package_name: &str,
    ports: &P,
    ...
    let Some(mut state) = read_installed_state(layout, package_name).await? else {
        tracing::info!("Skipped command-surface reapply for '{package_name}': not installed.");
        return Ok(());
    };
    let current_root = layout.package_current_root(package_name);
    if !current_root.exists() {
        tracing::info!("Skipped command-surface reapply for '{package_name}': current root missing.");
        return Ok(());
    }
    ...
    // Resolve manifest for fresh source data
    let resolved = crate::resolve_package_manifest(layout, package_name).await
        .ok_or(ScoopError::ManifestUnavailable)?;
    let manifest_value = crate::load_manifest_value(&resolved.manifest_path).await
        .map_err(|e| ScoopError::Other(format!("failed to load manifest: {e}")))?;
    let source = crate::resolve_package_source(&manifest_value)?;

    // Remove old shims from global shims dir
    for bin in &state.command_surface.bins {
        let shim_path = shims_root.join(format!("{bin}.cmd"));
        if shim_path.exists() {
    ...
    // Write new shims
    let _persist_root = layout.package_persist_root(package_name);
    let mut new_bins = Vec::new();

    for shim_target in &source.bins {
        let shim_path = shims_root.join(format!("{}.cmd", shim_target.alias));
        let shim_content = format!(
            "@echo off\nset __do_not_use__=1\nexe=\"%~dp0{}\"\nif exist \"%SCOOP_PERSIST_DIR%\\{}\" (\n  set \"SCOOP_PERSIST_DIR=%~dp0..\\..\\persist\\{}\"\n)\n\"%~dp0..\\{}\" %*\n",
            shim_target.relative_path.replace('/', "\\"),
            shim_target.relative_path.replace('/', "\\"),
        ...
        tokio::fs::write(&shim_path, shim_content).await
            .map_err(|e| ScoopError::fs("write", &shim_path, e))?;
        new_bins.push(shim_target.alias.clone());
    }

    // Add supplemental shims from ports
    for spec in ports.supplemental_shims(package_name, &current_root) {
        let shim_path = shims_root.join(format!("{}.cmd", spec.alias));
        let shim_content = format!(
            "@echo off\n\"%~dp0..\\apps\\{}\\current\\{}\" %*\n",
            package_name,
        ...
        tokio::fs::write(&shim_path, shim_content).await
            .map_err(|e| ScoopError::fs("write", &shim_path, e))?;
        if !new_bins.contains(&spec.alias) {
            new_bins.push(spec.alias);
        }
    }

    // Update state
    state.command_surface.bins = new_bins;
...
/// Reapply integrations for an installed package.
///
/// Reads the installed state, delegates to ports for integration execution,
/// and updates the install state.
pub async fn reapply_integrations<P: ScoopPorts>(
    layout: &ScoopLayout,
    package_name: &str,
    ports: &P,
    events: Option<&EventSender>,
    ...
    let Some(mut state) = read_installed_state(layout, package_name).await? else {
        tracing::info!("Skipped integration reapply for '{package_name}': not installed.");
        return Ok(Vec::new());
    };
    let current_root = layout.package_current_root(package_name);
    if !current_root.exists() {
        tracing::info!("Skipped integration reapply for '{package_name}': current root missing.");
        return Ok(Vec::new());
    }

    if let Some(sender) = events {
        sender.send(spoon_core::SpoonEvent::Stage(spoon_core::StageEvent::started(
            spoon_core::LifecycleStage::Integrating,
        )));
    }

    let persist_root = layout.package_persist_root(package_name);
    let integrations = ports.apply_integrations(package_name, &current_root, &persist_root)?;

    state.integrations = integrations.clone();
    write_installed_state(layout, &state).await?;

    if integrations.is_empty() {
        tracing::info!("Reapplied integrations for '{package_name}': no changes.");
    } else {
