...
pub fn normalize_proxy_url(proxy: &str) -> Result<Option<String>> {
    ...
    let parsed = Url::parse(&candidate)
        .map_err(|error| CoreError::Config(format!("invalid proxy '{trimmed}': {error}")))?;
    ...
    pub fn build(self) -> Result<reqwest::Client> {
        ...
        if let Some(proxy) = self.proxy {
            let proxy = Proxy::all(&proxy).map_err(|error| {
                CoreError::Config(format!("invalid proxy '{proxy}': {error}"))
            })?;
            builder = builder.proxy(proxy);
        }
        builder
            .build()
            .map_err(|err| CoreError::http_client("build", err))
    }
}
